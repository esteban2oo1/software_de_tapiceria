...
const InventoryPage = () => {
  ...
  const handleAddItem = async () => {
    try {
      ...
      const mapping = itemTypeMappings[newItemData.itemType];
      ...
      const requestBodyData = requestBody(newItemData);
  ...
  const handleProductTypeChange = async (event) => {
    ...
    try {
      ...
      if (!selectedType) {
        // If no type is selected, fetch data for all product types
        const allProductTypesData = await fetchAllProductTypesData();
        setProductTypeData(allProductTypesData);
      } else {
        // If a type is selected, fetch data for that specific type
        await handleFetchProductTypeData(selectedType);
      }
    } catch (error) {
      setError(error);
  ...
  const fetchExistenciasDataForType = async (productType) => {
    ...
    try {
      ...
      const mappedData = data.map((item) => ({
        id: item.id,
        itemName: getProductTypeItemName(productType, item), // Uso de la función aquí
        quantity: item.cantidad,
      }));
